...
export { MAX_RETRIES, ensureString, formatTaskStatus, formatBoardSummary, unblockReadyTasks, globOverlaps } from "./src/task-board"
...
export default (async (ctx) => {
  ...
  function checkConflict(pattern: string, agent: string): any | null {
    const exact = checkReservationExact.get({ $pattern: pattern, $agent: agent }) as any
    if (exact) return exact
    for (const r of getAllActiveReservationsExcept.all({ $agent: agent }) as any[]) {
    ...
    tool: {
      ...
      tb_claim: tool({
        ...
        async execute(args, context) {
          ...
          if (r.changes === 0) {
            ...
            const deps = JSON.parse(t.dependencies || "[]") as string[]
            const unmet = deps.filter((d: string) => { const x = getTask.get({ $id: d, $board_id: b.id }) as any; return x && x.status !== "completed" }).map((d: string) => { const x = getTask.get({ $id: d, $board_id: b.id }) as any; return `${d} (${x?.status})` })
      ...
      tb_unblocked: tool({
        ...
        async execute(args) {
          ...
          const ready = getUnblockedTasks.all({ $board_id: b.id }) as any[]
          if (!ready.length) {
            ...
            return `No unblocked tasks. ${rem.length} still blocked.`
          }
      ...
      file_reserve: tool({
        ...
        async execute(args, context) {
          ...
          for (const p of patterns) {
            const ex = checkConflict(p, agent) as any
      ...
      file_check: tool({
        ...
        async execute(args, context) {
          ...
          let patterns: string[]
          try { patterns = JSON.parse(args.patterns) } catch { patterns = [args.patterns] }
          return patterns.map(p => { const ex = checkConflict(p, agent) as any; return ex ? `🚫 ${p} — by @${ex.reserved_by} until ${ex.expires_at}` : `✅ ${p} — available` }).join("\n")
        },
      }),
